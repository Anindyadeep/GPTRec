...
def predict(request: Request, body: RecommendationRequest):
    try:
        if body.objs:
            ...
            query = body.text
        result = resources["client"].recommend(query, show_structured_json=False)
        return {
            "search_id": body.id,
...
def rec_similar(request: Request, body: RecommendationRequest):
    try:
        if body.objs:
            ...
            query = body.text
        result = resources["client"].recommend(query, flag="watch_history")
        return {
            "search_id": body.id,
            "response_id": str(uuid.uuid4()),
            "search_query": str(query),
